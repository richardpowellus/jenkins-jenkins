pipeline {
  ...
  stages {
      ...
      steps {
        script {
          NEW_UPSTREAM_DOCKERHUB_IMAGE_DIGEST = sh(
            script: '''
              docker manifest inspect jenkins/jenkins -v | jq '.[].Descriptor | select (.platform.architecture=="amd64" and .platform.os=="linux")' | jq -r '.digest'
            ''',
            returnStdout: true
          ).trim()
      ...
      steps {
        script {
          SECONDS_SINCE_LAST_IMAGE = sh(
            script: '''
              d1=$(curl -s GET https://hub.docker.com/v2/repositories/dprus/jenkins/tags/latest | jq -r ".last_updated")
    ...
    stage('Build') {
      steps {
        sh 'docker build -t dprus/jenkins:latest .'
      }
    }
       
    stage('Push image to Docker Hub') {
      steps {
        sh 'docker push dprus/jenkins:latest'
      }
    }
